...
// Decode a single utf32 code point from a utf8 string
uint32_t utf8_decode(const char** ptr)
{
    ...
    if (codepoint & 0x80)
    {
        ...
        {
            return 0xFFFFFFFF;      // Invalid code point
        }
        for (int i=0; i<n; i++)
        {
            uint8_t b = *p++;
...
static const uint16_t cvt1[] = {	/* Compressed up conversion table for U+0000 - U+0FFF */
    /* Basic Latin */
    0x0061,0x031A,
    /* Latin-1 Supplement */
    0x00E0,0x0317,
...
static const uint16_t cvt2[] = {	/* Compressed up conversion table for U+1000 - U+FFFF */
    /* Phonetic Extensions */
...
// Convert UTF32 code point to upper case
// (Based on implementation from FatFS)
uint32_t utf32_toupper(uint32_t codepoint)
{
    // Is it in BMP? 
	if (codepoint < 0x10000) 
    {	
		uint16_t uc = (uint16_t)codepoint;
		const uint16_t* p = uc < 0x1000 ? cvt1 : cvt2;
		while (1) 
        {
            // Get the block base
			uint16_t bc = *p++;	
